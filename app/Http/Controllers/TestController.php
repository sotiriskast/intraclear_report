...
//            $merchantIds = [8026, 8027, 8028,8003,8004];
            $merchantIds = DB::connection('payment_gateway_mysql')->table('account')->where('active',1)->pluck('id')->toArray();
            ...
            $accounts = DB::connection('payment_gateway_mysql')->table('account')
                ->select([
...
//                    'scheme_rates.sell as exchange_rate',
                    DB::raw('MIN(transactions.added) as transaction_date'), // Replace added field
...
//                    'scheme_rates.sell',
            ...
            $organized = $accounts->groupBy('account_id')->map(function ($merchantData) use ($exchangeRates) {
                            ...
                            foreach ($currencyData as $data) {
                                if ($currency === 'EUR') {
                                    ...
                                    if (empty($data->card_type)) {
                                        $this->logger->log('error',"Missing card_type for transaction on {$transactionDate}, currency: {$currency}");
                                        continue;
                                    }
                                    ...
                                    // Debug log
                                    $this->logger->log('debug',"Looking for rate with key: {$rateKey}");
    ...
    {
        try {
            $dailyRates = DB::connection('payment_gateway_mysql')
                ->table('scheme_rates')
                ->select([
                    'from_currency',
